...
class EditRecordType extends FormRequest
{
    /**
    ...
    public function rules()
    {
        ...
        return [
            'name'=> ['required', Rule::unique('record_types')->where('user_id', $user_id)
                ->where('account_type', $this->input('account_type'))->ignore($this->record_type->id)],
            'account_type'=> ['required', $user_id ? Rule::in([User::find($user_id)->account_type]) : 'in:personal,corporate'],
            'type_order' => ['required', Rule::unique('record_types')->where('user_id', $this->input('user_id'))
                ->where('account_type', $this->input('account_type'))->ignore($this->record_type->id) ],
        ];
    }
